...
[[noreturn]] void TrafficLight::cycleThroughPhases()
{
    ...
    // sends an update method to the message queue using move semantics. The cycle duration should
    // be a random value between 4 and 6 seconds. Also, the while-loop should use
    ...
    while (true) {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>
                (lastUpdate - timeSinceLastUpdate).count() >= dist(gen)) {
